...
module.exports = {
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("url").setDescription("Discord CDN or Plain Image URL")
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("name").setDescription("Filename (SHA256Sum)")
        )
    ),
  ...
  async execute(ctx: Ctx) {
    ...
    if (url) {
      ...
      ctx.logger.info(
        `attempting to write frog to disk as temp file ${tempPath}`
      );
      ...
      if (!reg.test(frogFileName)) {
        await ctx.interaction.reply(
          `${frogFileName} Does not pass validation, skipping. (check console)`
        );
      ...
      const existingFrogs = await fs.promises.readdir(
        path.resolve(__dirname, "../../frogs")
      );
      ctx.logger.info(`Checking if ${frogFileName} already exists on disk`);

      if (existingFrogs.includes(frogFileName)) {
        ctx.logger.info(
          `${frogFileName} Already exists, will not be added to disk`
        );
        ...
        const frogAcceptedPath = path.resolve(
          __dirname,
          "../../frogs",
          frogFileName
        );
    ...
    if (filename) {
      ...
      if (!reg.test(filename)) {
        await ctx.interaction.reply(
          `${filename} Does not validate safety checks. Sussy baka.`
        );
